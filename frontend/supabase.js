...
const storage = {
  ...
  async deleteImage(path) {
    await supabaseClient.storage.from('clothes-images').remove([path]);
  },
};
...
const db = {
  async getClothes(profileId) {
    const { data } = await supabaseClient.from('clothes').select('*').eq('profile_id', profileId).order('created_at', { ascending: false });
    return data || [];
  },

  async insertClothing(profileId, item) {
    const { data, error } = await supabaseClient.from('clothes').insert({
  ...
  async saveOutfit(profileId, tag, score, temp, clothingIds) {
    const { data: outfit } = await supabaseClient.from('outfit_history').insert({
      profile_id: profileId,
      contesto_tag: tag,
    ...
    if (outfit) {
      ...
      await supabaseClient.from('outfit_items').insert(items);
    }
    return outfit;
  },

  async getOutfitHistory(profileId, limit = 50) {
    const { data } = await supabaseClient.from('outfit_history').select('*').eq('profile_id', profileId).order('data_utilizzo', { ascending: false }).limit(limit);
    return data || [];
  },
};
